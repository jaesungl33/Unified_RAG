...
def normalize_doc_id_for_matching(doc_reference: str) -> str:
    ...
    correctly match doc_id "Asset_UI_Tank_War_In_game_GUI_Design" in Supabase.
...
def parse_section_targets(query: str) -> Tuple[str, Dict]:
    ...
    # For doc_ids: @[Asset,_UI]_[Tank_War]_filename.md
    ...
    # Strategy: Match @ followed by text, stopping at the space before the question
    ...
    #   |                   - OR
    #   \S+(?:\s+\S+)?      - 1-2 non-whitespace words (e.g., "Notes" or "Result Screen")
    # )                     - end capture group
    # \s                    - followed by whitespace (the delimiter before question)
    ...
    for match in matches:
        # Remove @match from query
        cleaned_query = cleaned_query.replace(f'@{match}', '').strip()
        
        # Try to match to section_path or doc_id
        # NOTE: Content type filtering removed - @ syntax now only supports sections
        
        # Check if it looks like a doc_id pattern (contains brackets, multiple underscores, or .md)
        # Be more strict: require brackets OR multiple underscores OR .md extension
        is_doc_id = (
